...
def write_excel(swfcn_clean_result, swf_clean_result, filename, ESTAND):
    try:
        ...
        start_row = ws.max_row + 1  # 数据起始行（一般是 2）
            ...
            for build_type, categories in build_types.items():
                for category, swfcn_urls in categories.items():
                    ...
                    for idx, swfcn_url in enumerate(swfcn_urls):
                        ...
                        ws.append([
                            "8295_master",   # A Date/Branch
                            ESTAND,          # B Version/Pipeline
                            star_version,    # C STAR Version（如需不同可自行调整）
                            build_type,      # D Build Type
                            category,        # E Archive Name
                            swfcn_url,       # F
                            swf_url           # G
                        ])
        ...
        # ========== 合并 C 列（STAR Version） ==========
        merge_same_cells(ws, col=3, start_row=2)

        # ========== 合并 D 列（Build Type） ==========
        merge_same_cells(ws, col=4, start_row=2)

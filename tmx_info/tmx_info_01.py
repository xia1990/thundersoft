...
def get_tmx_info(issues):
    ...
    for issue in issues:
        ...
        if version_type == "STAR3.0" or version_type == "STAR3.5":
            # print(f"版本类型是: {version_type}")
...
def write_excel(issues, tmx_info):
    ...
    sheet_configs = {
        "STAR3.0": {
            ...
            "headers": ["i3 RSU Star 3.0 Dev", "Passed", "Total"],
            "col_counter": 1
        },
        "STAR3.5": {
            "ws": ws2,
            # "headers": ["i3 RSU Star 3.5", "Passed", "Blocked", "Failed", "NT", "Total", "MR Number"],
            "headers": ["i3 RSU Star 3.5 Dev", "Passed", "Total"],
            "col_counter": 1
        },
        "default": {
            "ws": ws3,
            # "headers": ["RSU VIP", "Passed", "Blocked", "Failed", "NT", "Total"],
            "headers": ["RSU VIP", "Passed", "Total"],
            "col_counter": 1
        }
    }
    ...
    for issue in issues:
        ...
        version = build_type.split("_")[:-1]
        print(version_type, version)
        # 只提取DEV的测试数据
        if build_type.endswith("DEV"):
            # mr_label = parts[1]
            ...
            ws.cell(row=3, column=col_index, value=data.get("total", 0))
            # ws.cell(row=4, column=col_index, value=mr_number)
